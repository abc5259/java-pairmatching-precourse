...
- [x] resources 파일을 이용해 크루 정보를 초기화 한다.
- [x] 기능 선택 화면을 출력한다.
- [x] 과정, 레벨, 미션에 맞는것이 있는지 찾는다.

### 페어매칭 기능

- [x] 과정와 미션을 출력하고 매칭하고자 하는 과정, 레벨, 미션을 입력 받는다.
- [x] 페어매칭 이력이 없는 경우 페어를 매칭한다.
- [x] 페어매칭 이력이 있는 경우 다시 매칭할건지 입력받는다.
- [x] 페어매칭을 알고리즘에 따라 구현한다.
    - [x] 추가고려사항. 3회 시도까지 매칭이 되지 않거나 매칭을 할 수 있는 경우의 수가 없으면 에러 메시지를 출력한다.
- [x] 페어매칭 결과를 출력한다.

### 페어조회 기능

- [x] 과정와 미션을 출력하고 조회하고자 하는 과정, 레벨, 미션을 입력 받는다.
- [x] 매칭 이력이 없다면 안내문구를 출력한다.
- [x] 매칭된 이력을 출력한다.

### 페어 초기화 기능

- [x] 이때까지 매칭된 페어를 모두 초기화한다.
